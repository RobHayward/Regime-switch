...
It is possible to look at the evolution of most likely regimes over time.  Figure \ref{figref:huf} shows the potential carry-trade profits and the two regimes for Poland.  It is clear that the Global Financial Crisis is the main regime change that takes place.  However, there are a number of smaller changes that take place. 
%\begin{figure}[!h]
%\centering
