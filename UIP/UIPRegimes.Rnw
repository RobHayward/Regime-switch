...
\maketitle
\begin{abstract}
The common observation that the future exchange rate deviates systematically from the forward rate and that excess returns are available from borrowing low interest rate currencies and depositing in higher rate units is called the forward bias puzzle. This paper adds to accumulating evidence that the apparent excess returns are more likely a compensation for taking crash-risk. A range of Central and Eastern European (CEE) exchange rates are analysed. A Hidden Markov Model (HMM) is used to identify two regimes for most of the exchange rates.  These two regimes can be characterised as either periods of 'calm' or periods of 'crisis. The level of international risk aversion and changes in US interest rates affect the probability of switching from one regime to the other. This model is then used to assess the way that these two factors affect the probability of a currency crisis. While the Czech Republic,  Hungary and Bulgaria are very sensitive to international financial conditions, Poland and Romania are relatively immune.  
...
If it is assumed that expectations are formed rationally and there are no financial frictions, the forward rate should be the best, unbiased estimate of the future spot rate and therefore, Equation \ref{eq:fw} becomes.  
...
where $rp_{t, j}$ is the risk premium at time $t$ for $j$ periods ahead.  This seemed reasonable for the original US-centric research but it means that the risk premium is on domestic assets for non-US investors and that the risk premium switched to US assets when US interest rates rose.  There are other problems with this approach. \citep{CanovaMarrinan} find that the risk premium switches from positive to negative as relative interest rates change and that the risk premium has a large variance, serial correlation and has hetroskedasticity due to volatility clustering.  Indeed, Froot shows that short rates consistently predict excess returns on a variety of assets.  For foreign exchange, stock, bond and commodity markets, a one percentage annualised increase in the short-term interest rate is associated with about a three percentage point reduction in annualised excess returns  \citep{FrootUIP}. 

However, the calculated risk premium is a risk premium only if there are rational expectations. For this $E(e_{t+1}) = E^m(s_{t+1})$. \citep{Engel1996}

 An alternative approach will consider the difference between the forward rate and the change in the exchange rate as the \emph{excess return}
\begin{equation}
rp^{re} \equ f_t - E(s_{t+1})
\end{equation}

Market expectations may differ from the information that is used to test Equation \ref{eq:uip2}. There may be a difference between the Engel offers two cases:  where the econometrician has more information that the agent (agent gradually learn the model) or where the agent has information that the econometrican does not have (the peso problem) \citep{Engel1996}.  In each of these cases it is usual to expect that the difference should tend to zero over time as market participants or economists acquire more information.  

The alternative approaches will view the difference between the forward rate and the change in the exchange rate as either being explained by a stochastic discount factor.  There are a number of models that seek to assess the excess returns that are possible from an investment in the exchange rate relative to the forward rate. These are sometime called the \emph{Latent variables model}. Here the excess returns are \emph{explained} by the latent variables. The usual instruments are forward discounts.  \citep{Burnside2010} assess whether carry-returns are correlated with conventional risk factors like the equity risk-premium or real consumption growth and volatility indices.  They find limited correlation.\citep{McCallum} assumes that $rp^{re}$ is a stochastic process and ask whether there are reasonable grounds for this to be negatively correlated with $E(s_{t+1}-s_t)$. He concludes that this residual represents all the factors that are not in the model rather than a risk premium.   


\subsection{Crash risk}\label{secref:crash}
It is also possible to abandon the assumption that markets are efficient and to view the risk premium as the excess returns that are available for a carry trade.  Engel says that $rp^{re}$ could represent 'expected profit opportunities' (p148). As has been seen, these expected profit opportunities are large and highly variable. 
...
There are a number of studies that have assessed the skewed nature of the returns that are associated with the carry trade.  For example, \citep{BrunnermeierCarry}develop a general model of what they call \emph{crash risk}, which is due to the sudden unwinding of carry trade.  The crash happens when risk-appetite and funding-liquidity decrease. A potential carry-trade is calculated 
...
Gabrisch and Orlowski investigate time series properties of equity markets, money market rates and exchange rates in some CEE countries using a GARCH framework to assess the nature of the disturbances in these time series.  It is clear that fat-tailed, negative-skewed distributions are a common feature of CEE financial markets.  Indeed, it appears from this study that foreign exchange shocks, such as the effects of the carry trade, translate across other financial markets \citep{GabrischOrlowski}.   
...
The approach adopted here is to assess whether carry-trade excess returns are best modelled as stemming from one or more regimes. In theory, the returns to the carry-trade would depend on whether international investors considered conditions to be calm, where they would be willing to build carry-trade exposure, or in crisis, where they would reduce positions as swiftly as possible.  A model with more than one regime is often called a \emph{mixture model} as the population of carry-trade returns are assumed to come from two or more unobserved sub-populations. As we are using time series and it is assumed that periods of calm are most likely to grouped together and that exchange rate crises come as a cluster a Markov chain can be used to model the tradition from one period to another.  As the periods of calm and crisis are not observed, this is called a \emph{Hidden Markov Model}.  
...
It is also necessary to iterate backwards to assess the probability that a particular sequence will be observed from a point in time to the end of the sequence.  This is based on $\beta_t(i) = P(Y_{t+1}, Y_{t+2}\dots Y_T| S_t = S_i, \lambda)$. Setting the end probability as unity and inducting backwards, 

% I have been asked to put more commentary in this section. 
\begin{equation} 
...
where $i_t^*$ is the investment currency, $i_t$ is the funding currency, $\Delta s_t$ is the change in the exchange rate and $p_t$ is the gross profit for potential carry-trades without transaction costs or adjustment for risk.  \citep{Burnside2010} constructs a similar series for a panel of 10 exchange rates and finds a very small effect from plausible transaction costs and Sharpe Ratios that compare favourably with equity markets.  One month positions are calculated.    

\subsection{Model Comparison}
There are four models that are applied to each of the carry-trade-profit series. 
\begin{itemize}
\item Model 1 (M1) is a standard linear model $p_t = \beta_0  + \varepsilon_t$ where $p_t$ is the carry-trade-profit series, $beta_0$ is the intercept and $\varepsilon_t$ is the variation around that level.  In other words, it provides a provides a mean for the carry-trade-profit for the whole series and a calculation for how the series varies around that level. 
\item Model 2 (M2) standard linear model with two regimes $p_t = \beta_0 + \beta_1 S_n + \varepsilon, \quad n = 1, 2$ there is a linear model with two regimes, therefore there is an average value and a standard deviation for each of the two regimes. 
\item Model 3 (M4) standard linear model with three regimes $p_t = \beta_0 + \beta_1 S_n + \varepsilon, \quad n = 1, 2, 3$ average and standard deviations are retried for three different regimes. 
\end{itemize}
...
There are two other things to note.  First, the funding currency that is selected does not make much difference.   The average performance of stability and crash regimes across funding currencies (final column of Table \ref{tabref:2stateresponse}) does not differ significantly.  The rest of the results that are presented are based on EUR-funding.  Secondly, the mean and standard deviation for those countries where the Information criteria and log-likelihoods raised doubt about the two-regime also suggest that a one-regime model may be more appropriate,   For the Czech Republic, however, there remains ambiguity and uncertainty. Outside of the JPY funding, there are two clear regimes identified by the mean and standard deviations. 
...
&&S-Dev& 0.0307 & 0.0297 & 0.0305 & 0.0080 & 0.0095 & 0.0314 & 0.0189 202 \\ 
  &\multirow{2}{*}{Crash}&Mean& 0.9925 & 0.9845 & 0.9983 & 1.0052 & 1.0004 & 1.0034 & 1.0016 & 0.9959 \\ 
   & & S-Dev & 0.0707 & 0.0641 & 0.0493 & 0.0389 & 0.0407 & 0.0792 & 0.0413 & 0.0635 & 0.0390 & 0.0566 \\ 
\hline
\multirow{4}{*}{CHF}& \multirow{2}{*}{Stable}&Mean& 1.0052 & 1.0097 & 1.0040 & 1.0085 & 1.0033 & 1.0099 & 1.0012 & 1.0051 \\ 
...
\hline  
  \multirow{4}{*}{JPY}&\multirow{2}{*}{Stable}& Mean& 1.0149 & 1.0157 & 1.0125 & 1.0149 & 1.0074 & 1.0091 & 1.0115 \\ 
 & & S-Dev &0.0348 & 0.0359 & 0.0241 & 0.0291 & 0.0226 & 0.0401 & 0.0191 & 0.0307 & 0.0210 & 0.0289 \\ 
  & \multirow{2}{*}{Crash}&Mean & 0.9843 & 0.9727 & 1.0012 & 0.9972 & 0.9983 & 1.0061 & 1.0002 & 0.9801 \\ 
...
\subsection{Determinants of exchange rate crisis}
Unwinding of carry-trades and exchange rate crisis can be caused by domestic or international factors.  Two exogenous international international factors that may increase the risk of a currency crisis have been identified as international risk aversion and changes in US interest rates. Flight-to-Safety (FTS) is the term used to describe capital flows towards international financial centres and relatively safe assets, \citet{FTS} discover that FTS are relatively rare events. Their study of of 23 countries finds that most of the events are country specific. They characterise only 25\% of the events as "global", which means that they are associated with an increase in the VIX index and the TED spread.\footnote{The VIX is an index of implied volatility on options from the S\&P 500 index.  It is commonly used as a measure of international risk aversion as it signals increased demand by fund managers for option protection of their equity portfolio.  See \citet{VIX}, \citet{GoldmanVol} and \citet{Diamond} for fuller details.The TED spread is the spread between the treasury bill and the euro-dollar rate.  It is used as a market measure of perceived credit risk of financial institutions as it records the risk premium that investors require to lend to banks relative to the risk-free rate.}   
