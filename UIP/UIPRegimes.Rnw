...
The presence of a \emph{risk premium} is one of the main explanations put forward to explain this puzzle. If investors are assumed to be risk averse, the interest differential will be equal to the expected change in the exchange rate plus an additional premium for taking risk.   In that case the negative estimate for $\beta$ could mean that investors required additional return for holding foreign currency assets and Equation \ref{eq:uip} is augmented with an additional term that would account for the risk premium.  
