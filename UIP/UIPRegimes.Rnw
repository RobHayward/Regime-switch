...
If it is assumed that expectations are formed rationally and there are no financialk frictions, the forward rate should be the best, unbiased estimate of the future spot rate and therefore, Equation \ref{eq:fw} becomes.  
...
where $rp_{t, j}$ is the risk premium at time $t$ for $j$ periods ahead.  Again, $\beta_0$, $\beta_1$ and $\beta_2$ are the parameters of the model (to be estimated) and $\beta_0$ should be equal to zero while $\beta_1$ and $\beta_2$ should sum to one so that, on average, any discrepancy between the forward rate and the expected change in the exchange rate is offset by the risk premium.  This seems reasonable for the original US-centric research but it means that the risk premium is on domestic assets for non-US investors and that the risk premium switched to US assets when US interest rates rose.  Theoretically and empicially, the calculated rational expectations risk premium has a large variance. For example, \citep{CanovaMarrinan} find that the risk premium switches from positive to negative as relative interest rates change and that the risk premium has a large variance, serial correlation and has hetroskedasticity due to volatility clustering.  Indeed, Froot shows that short rates consistently predict excess returns on a variety of assets.  For foreign exchange, stock, bond and commodity markets, a one percentage annualised increase in the short-term interest rate is associated with about a three percentage point reduction in annualised excess returns  \citep{FrootUIP}. 

\subsection{Beyond the risk premium}
It is debated whether the space between the forward rate and the actual change in the exchange rate should be considered as a risk premium or an execess return.  Engel says that the calculated risk premium is a risk premium only if there are rational expectations. For this $E(e_{t+1}) = E^m(s_{t+1})$. \citet{Engel1996}

\begin{equation}
rp^{re} \equ f_t - E(s_{t+1})
\end{equation}

Market expectations may differ from the information that is used to test Equation \ref{eq:uip2}. Engel offers two cases:  where the econometrician has more information that the agent (agent gradually learn the model) or where the agent has information that the econometrican does not have (the peso problem).  In each of these cases the difference should tend to zero over time as market participants or economists acquire more information.. 
...
\citet{McCallum} assumes that $rp^{re}$ is a stochasitic process and ask whether there are reasonable grounds for this to be negatively correlated with $E(s_{t+1}-s_t)$. He concludes that this residual represents all the factors that are not in the model rather than a risk premium.  
...
The approach adopted here is to assess whether carry-trade excess returns are best modelled as steming from one or more regimes. In theory, the returns to the carry-trade would depend on whether international investors considered conditions to be calm, where they would be willing to build carry-trade exposure, or in crisis, where they would reduce positions as swiftly as possible.  A model with more than one regime is often called a \emph{mixture model} as the population of carry-trade returns are assumed to come from two or more unobserved sub-populations. As we are using time seies and it is assumed that periods of calm are most likely to grouped together and that exchange rate crises come as a cluster a Markov chain can be used to model the trasition from one period to another.  As the periods of calm and crisis are not observed, this is called a \emph{Hidden Markov Model}.  
