...
Give this information, it is possible to construct a sample of carry-returns from the initial probabilities, the transition matrix and the emission matrix.  Alternatively, it is possible to work backwards from the carry returns using the  transiton matrix and the emission matrix to find the most likely sequence of financial states by applying the \emph{viterbi} algorithm (see Chapter10.R for full details of how this works).  This will essentially. 

I think that the expectatiosn-? algorithm will calculate the expected hidden sequence given the estimated parameters and will then optimise the the parameters give the sequence. However, I wll have to check. 
