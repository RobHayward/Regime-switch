...
There is evidence that the international financial cycle is increasingly global \citet{Rey2013}, \citet{Obstfeld2013} and \citet{Bruno2014}.  %\href{http://www.voxeu.org/article/primer-global-liquidity}{VOX: GLobal Financial Liquidity}. 
...
%Equation \ref{eqref:m1} shows the transition matrix for the transition model, The first row shows the probabilities of remaining in a state of calm is this is the current state and moving from a state of calm to a state of crisis.  These are 0.8 and 0.2 respectively.  The second row shows the probability of starting in a state of crisis and moving to calm or remaining there.  The model does not allow a switch back to calm so the first column is zero and the second is one. 
