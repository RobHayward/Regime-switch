Notes
========================================================
[Nature](http://www.nature.com/nbt/journal/v26/n8/full/nbt1406.html)
What is the *expectations-maximisation* alogrithm? The example is one of coin-flipping.  The *bias* can be estimated by maximum likelihood. Maximum-likelihood in this case is the validity of the model (including the estimate of the parameters) given the data. 

If data is incomplete, with the coin or regime used missing, the expectattion iterates between guessing a probability distribution over completions or regimes of missing data given the current model and then re-estimating the model parameters using these completions or regime (maximisation). 

The E-M alogrithm computes the probability of the regime given the initial parameters.  For the hudden markov model, the probability of switching from one regime to another must also be computed. 
